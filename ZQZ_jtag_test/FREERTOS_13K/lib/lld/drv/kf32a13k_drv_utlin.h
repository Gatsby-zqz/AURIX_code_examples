...
#define CHECK_UTLIN_FLAG(Flag)                                                                                         \
    ((Flag == SEND_FRAMEHEAD_END_IF) || (Flag == SEND_FRAMRESPONSE_END_IF) || (Flag == RECEIVE_FRAMEHEAD_END_IF) ||    \
     (Flag == RECEIVE_FRAMERESPONSE_END_IF) || (Flag == DETECT_FAILEDGE_IF) || (Flag == DETECT_RISEEDGE_IF) ||         \
...
Utlin_LL_SetOcdsSuspendMode(Utlin_RegisterMap_t *const Module, const Utlin_OcdsSuspendMode_t OcdsSuspendMode)
{
    ...
    UTLIN_OCDS_T regValue = {0U};
    regValue.reg = REG_READ(Module->UTLIN_OCDS.reg);
    regValue.bits.SUS_P = true;
    regValue.bits.SUS = OcdsSuspendMode;
    REG_WRITE(Module->UTLIN_OCDS.reg, regValue.reg);
}
