...
#ifdef SIMULATION
#define CRC_MODULE_BASE_ADDR ADCS_ADDR
#include "kf32a13k_reg_simulation_adcs.h"
#endif
...
KF_INLINE void Adcs_LL_SetSuspendMode(const Adcs_Module_t AdcModule, Adcs_SuspendMode_t SuspendMode)
{
    ...
    ADCS_DBG_T regValue = {0U};

#if SIMULATION
    regValue.reg        = REG_READ(HWIO_ADC0_DBG_ADDR + (4U * AdcModule));
    regValue.bits.SUS_P = true;
    regValue.bits.SUS   = SuspendMode;
    REG_WRITE(HWIO_ADC0_DBG_ADDR + (4U * AdcModule), HWIO_ADC0_DBG_ADDR + (4U * AdcModule));
#else
