...
#endif
#ifdef SIMULATION
#define GPIO_MODULE_BASE_ADDR GPIO0_ADDR
#include "kf32a13k_reg_simulation_gpio.h"
#endif
#ifdef __cplusplus
extern "C" {
#endif
...
{
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_INFO_ADDR + offsetAddr);
    regValue &= HWIO_GPIO0_INFO_GPIO0_INFO_MODREV_MASK;
    regValue = regValue >> HWIO_GPIO0_INFO_GPIO0_INFO_MODREV_SHFT;
    return ((regValue));
...
KF_INLINE uint32_t Gpio_LL_ReadModuleType(Gpio_RegisterMap_t *const Module)
{
    ...
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_INFO_ADDR + offsetAddr);
    regValue &= HWIO_GPIO0_INFO_GPIO0_INFO_MODTYPE_MASK;
    regValue = regValue >> HWIO_GPIO0_INFO_GPIO0_INFO_MODTYPE_SHFT;
    return ((regValue));
//TODO: check if correct
...
KF_INLINE uint32_t Gpio_LL_ReadModuleNumber(Gpio_RegisterMap_t *const Module)
{
    ...
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_INFO_ADDR + offsetAddr);
    regValue &= HWIO_GPIO0_INFO_GPIO0_INFO_MODNUMBER_MASK;
    regValue = regValue >> HWIO_GPIO0_INFO_GPIO0_INFO_MODNUMBER_SHFT;
    return ((regValue));
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
	return(REG_BITS_READ(offsetAddr + HWIO_GPIO0_ACES0_ADDR, MASK_1_BIT << TagId, TagId));
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_ACES0_ADDR + offsetAddr);
    regValue &= ~ MASK_1_BIT << TagId;
    regValue |=  Status << TagId;

    REG_WRITE(offsetAddr + HWIO_GPIO0_ACES0_ADDR , regValue);
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    return (REG_BITS_READ(offsetAddr + HWIO_GPIO0_PIR_ADDR, MASK_1_BIT << PinIndex, PinIndex));
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    return (REG_BITS_READ(offsetAddr + HWIO_GPIO0_POR_ADDR, MASK_1_BIT << PinIndex, PinIndex));
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_POR_ADDR + offsetAddr);
    regValue &= ~ MASK_1_BIT << PinIndex;
    regValue |=  Status << PinIndex;

    REG_WRITE(offsetAddr + HWIO_GPIO0_POR_ADDR , regValue);
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    Reg     = REG_READ(offsetAddr + HWIO_GPIO0_POR_ADDR) ^ PinMask;
//TODO: check if correct
...
#endif
#ifdef SIMULATION
     offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    REG_WRITE(offsetAddr + HWIO_GPIO0_POR_ADDR ,  Reg);
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_POSCR_ADDR + offsetAddr);
    regValue &= ~ MASK_1_BIT << (PinIndex + POS_16_OFFSET);
    regValue |=  StatusPc << (PinIndex + POS_16_OFFSET);
    REG_WRITE(offsetAddr + HWIO_GPIO0_POSCR_ADDR , regValue);
//TODO: check if correct
...
#ifdef SIMULATION
     offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
     regValue=REG_READ(HWIO_GPIO0_POSCR_ADDR + offsetAddr);
    regValue &= ~ MASK_1_BIT << PinIndex;
    regValue |=  StatusPs << PinIndex;
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_PUDEN_ADDR + offsetAddr);
    regValue &= ~ MASK_2_BIT << (PinIndex * POS_2_SHIFT);
    regValue |=  PinPuPdState << (PinIndex * POS_2_SHIFT);
    REG_WRITE(offsetAddr + HWIO_GPIO0_PUDEN_ADDR , regValue);
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    return ((Gpio_PinPuPdState_t)REG_BITS_READ(offsetAddr + HWIO_GPIO0_PUDEN_ADDR, MASK_2_BIT << (PinIndex * POS_2_SHIFT), PinIndex * POS_2_SHIFT));//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    return (REG_BITS_READ(offsetAddr + HWIO_GPIO0_PODR_ADDR, MASK_1_BIT << PinIndex, PinIndex));
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_PODR_ADDR + offsetAddr);
    regValue &= ~ MASK_1_BIT << PinIndex;
    regValue |=  Status << PinIndex;

    REG_WRITE(offsetAddr + HWIO_GPIO0_PODR_ADDR , regValue);
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_PMOD_ADDR + offsetAddr);
    regValue &= ~ MASK_2_BIT << (PinIndex * POS_2_SHIFT);
    regValue |=  PinOutputEn << (PinIndex * POS_2_SHIFT);
    REG_WRITE(offsetAddr + HWIO_GPIO0_PMOD_ADDR , regValue);
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_PEROUT0_ADDR + offsetAddr);
    regValue &= ~ MASK_4_BIT << (PinIndex * POS_4_SHIFT);
    regValue |=  PinRemap << (PinIndex * POS_4_SHIFT);
    REG_WRITE(offsetAddr + HWIO_GPIO0_PEROUT0_ADDR , regValue);
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_PEROUT1_ADDR + offsetAddr);
    regValue &= ~ MASK_4_BIT << (PinIndex * POS_4_SHIFT);
    regValue |=  PinRemap << (PinIndex * POS_4_SHIFT);
    REG_WRITE(offsetAddr + HWIO_GPIO0_PEROUT1_ADDR , regValue);
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_PDR0_ADDR + offsetAddr);
    regValue &= ~ MASK_2_BIT << (PinIndex * POS_4_SHIFT);
    regValue |=  PadDriverMode << (PinIndex * POS_4_SHIFT);
    REG_WRITE(offsetAddr + HWIO_GPIO0_PDR0_ADDR , regValue);
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_PDR1_ADDR + offsetAddr);
    regValue &= ~ MASK_2_BIT << (PinIndex * POS_4_SHIFT);
    regValue |=  PadDriverMode << (PinIndex * POS_4_SHIFT);
    REG_WRITE(offsetAddr + HWIO_GPIO0_PDR1_ADDR , regValue);
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_ESR_ADDR + offsetAddr);
    regValue &= ~ MASK_1_BIT << PinIndex;
    regValue |=  Status << PinIndex;

    REG_WRITE(offsetAddr + HWIO_GPIO0_ESR_ADDR , regValue);
//TODO: check if correct
 ...
 *
 * @param Module
 * @param LockValue 配置的锁定值
 * @retval void
 */
KF_INLINE void Gpio_LL_WriteLockStatus(Gpio_RegisterMap_t *const Module, const uint32_t LockValue)
{
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    REG_WRITE(offsetAddr + HWIO_GPIO0_LOCK_ADDR ,  LockValue);
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_PCSR_ADDR + offsetAddr);
    regValue &= ~ MASK_1_BIT << PinIndex;
    regValue |=  Status << PinIndex;

    REG_WRITE(offsetAddr + HWIO_GPIO0_PCSR_ADDR , regValue);
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_INEN_ADDR + offsetAddr);
    regValue &= ~ MASK_1_BIT << PinIndex;
    regValue |=  Status << PinIndex;

    REG_WRITE(offsetAddr + HWIO_GPIO0_INEN_ADDR , regValue);
//TODO: check if correct
...
#ifdef SIMULATION
    uint32_t offsetAddr = ((uint32_t)((uintptr_t)Module)) & ~GPIO_MODULE_BASE_ADDR;
    uint32_t regValue=REG_READ(HWIO_GPIO0_PUDRST_ADDR + offsetAddr);
    regValue &= ~ MASK_1_BIT << PinIndex;
    regValue |=  Status << PinIndex;

    REG_WRITE(offsetAddr + HWIO_GPIO0_PUDRST_ADDR , regValue);
//TODO: check if correct
